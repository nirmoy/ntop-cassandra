...
static void cassandraDumpRecord( char * buffer, int recordId);
...
u_char static cassandraDump = 0;
u_char static isCassandraActive = 1;
static char keyspace[48],
			columnfamily[48],
			server_IP[20];
  ...
  if((the5Record.flowHeader.version == htons(9))
    ...
    */
    ...
    for(i=0; i<numFlows; i++) {
	  ...
	  cassandraDumpRecord((char*) &the5Record.flowRecord[i], i);
...
static void
cassandraDumpRecord( char * buffer, int recordId)
{
  char time_now[32];
  char col_name[32];
  int ret;
  if (!cassandraDump) return;
  if (!isCassandraActive) return;
  if (checkCassandra(server_IP) == -1) {
	  traceEvent(CONST_TRACE_ERROR, "THREADMGMT[t%lu]: NETFLOW:"
                 "cassandra Server is not running", 
				 (long unsigned int)pthread_self());
	  return;
   }
  safe_snprintf(__FILE__, __LINE__, time_now, 32, "%u", time(NULL));
  safe_snprintf(__FILE__, __LINE__, col_name, 32, "%d", recordId);
 
  ret = cassandraEasyInsert(keyspace, columnfamily, server_IP, time_now,
					  col_name, buffer);
  if (ret != 0)
	  traceEvent(CONST_TRACE_ERROR, "THREADMGMT[t%lu]: NETFLOW:"
				 "Fail to store record in cssandra: %s", 
				 (long unsigned int)pthread_self(), getError());
   if (myGlobals.runningPref.debugMode)
	   traceEvent(CONST_TRACE_INFO, 
				  "NETFLOW: cassandraDumpRecord():ksp=%s cf=%s svr=%s row=%s",
				   keyspace, columnfamily, server_IP, time_now);
}
...
/**    Checking packet processing time  **/

    timestamp_t t2 =get_timestamp();
    double secs = (t2-t1)/1000000.0L;
    int temp =  myGlobals.device[deviceId].netflowGlobals->numNetFlowsPktsRcvd;
    total_time+= secs;
...
static int initNetFlowFunct(void) {
  ...
  if (cassandraInit(myGlobals.pluginDirs[0]) == -1) {

	  traceEvent(CONST_TRACE_ERROR, 
				 "NETFLOW : Fail to Initialize Cassandra Client: %s", 
				 getError());
	  isCassandraActive = 0;
   }

  if (isCassandraActive && isExistPycassa() == 0) {
...
#define UDPSLASHSCPT "UDP"
  ...
  sendString("<tr><th rowspan=\"3\" "DARK_BG">Flow or Record  Dump</th>\n");
  ...
  sendString("<tr><th "DARK_BG">Dump Records Cassandra-store</th>\n");
  sendString("<td "TD_BG"><form action=\"/" CONST_PLUGINS_HEADER);
  sendString(netflowPluginInfo->pluginURLname);
  sendString("\" method=GET>\n");
  safe_snprintf(__FILE__, __LINE__, buf, sizeof(buf),
                ...
                myGlobals.device[deviceId].netflowGlobals->netFlowDeviceId);
  sendString(buf);
  sendString("<input name=\"netflowCassandraDump\" size=\"60\" value=\"");

  sendString("\"> <input type=\"submit\" value=\"Set Dump File Path\">"
             "</p>\n</form>\n"
             "Specify Server IP:port, keyspace, columnfamily"
             "Ex:CASSA127.0.0.1:9160_testkeyspace_testcf</p>\n</td></tr>\n");
...
static void handleNetflowHTTPrequest(char* _url) {
  ...
  if((_url != NULL) && pluginActive) {
	...
	if(strcmp(device, "device") == 0) {
	  ...
	  if(deviceId > 0) {
	    ...
	    storePrefsValue(nfValue(deviceId, "netFlowDumpPath", 1), value);
	...
	} else if(strcmp(device, "netflowCassandraDump") == 0) {
      	if(deviceId > 0) {
	     ...
	     sscanf(value, "%[^_]_%[^_]_%[^\n]",server_IP, keyspace, 
				columnfamily);
         setPluginStatus(NULL);
		 cassandraDump = 1;
		 if ( !(server_IP[0] && keyspace[0] && columnfamily[0])) {
			 setPluginStatus("Invalid Cassandra Dump Entry");
			 traceEvent(CONST_TRACE_INFO, 
						"NETFLOW: Invalid Cassandra Dump Entry on Device:%d",
						 deviceId);
			 cassandraDump = 0;
		 }
         storePrefsValue(nfValue(deviceId, "netflowCassandraDump", 1), value);
        }
      }
      }
